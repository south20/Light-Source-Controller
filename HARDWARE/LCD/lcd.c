...
void LCD_ShowChar(u16 x,u16 y,u16 num,u16 size,u8 mode)
{  							  
    u16 temp,t1,t;
	u16 y0=y;
	...
	{   
		...
		else if(size==128)temp=asc2_12864[num][t];	//调用3216字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(u16 x,u16 y,u32 num,u16 len,u16 size,u8 mode)
{  
	u16 t,temp;
	u16 enshow=0;						   
	for(t=0;t<len;t++)
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u16 size,u8 *p)
{         
	u8 x0=x;
	width+=x;
	height+=y;
    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
    {       
		//printf("LCD_ShowString：x = %d,y = %d,width = %d,height = %d,size = %d...\r\n",x, y, width, height, size);
