...
void canDispatch(CO_Data* d, Message *m)
{
	 ...
	 switch(cob_id >> 7)
	{
		case SYNC:		/* can be a SYNC or a EMCY message */
			//printf("Hello This is SYNC ...\r\n");
			if(cob_id == 0x080)	/* SYNC */
			{
				if(d->CurrentCommunicationState.csSYNC)
					proceedSYNC(d);
				//printf("Hello This is CAN 0x080...\r\n");
			} else 		/* EMCY */
		...
		case PDO4tx:
		case PDO4rx:
			//printf("Hello This is PDO ...\r\n");
			if (d->CurrentCommunicationState.csPDO)
				proceedPDO(d,m);
			break;
		case SDOtx:
		case SDOrx:
			//printf("Hello This is SDO ...\r\n");
			if (d->CurrentCommunicationState.csSDO)
				proceedSDO(d,m);
			break;
		case NODE_GUARD:
			//printf("Hello This is NODE_GUARD ...\r\n");
			if (d->CurrentCommunicationState.csLifeGuard)
				proceedNODE_GUARD(d,m);
			break;
		case NMT:
			//printf("Hello This is NMT ...\r\n");
			if (*(d->iam_a_slave))
			{
...
void setNodeId(CO_Data* d, UNS8 nodeId)
{
  ...
  if(!(nodeId>0 && nodeId<=127)){
	...
	//printf("0x2D01 Invalid NodeID = %d.\r\n",nodeId);
