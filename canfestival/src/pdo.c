          ...
          if (getODentry (d, index, subIndex, tmp, &ByteSize, &dataType, 0) !=
              OD_SUCCESSFUL)
            {
              MSG_ERR (0x1013,
                       ...
                       (UNS32) (*pMappingParameter));
			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1013 Couldn't find mapped variable at index-subindex-size : = %lu.\r\n", (unsigned long)(*pMappingParameter));
              return 0xFF;
            }
...
sendPDOrequest (CO_Data * d, UNS16 RPDOIndex)
{
  ...
  //printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1931 sendPDOrequest : RPDO Index not found  : = %d.\r\n", RPDOIndex);
  return 0xFF;
}
                    ...
                    if (pMappingParameter == NULL)
                      {
                        MSG_ERR (0x1937, "Couldn't get mapping parameter : ",
                                 numMap + 1);
  						//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1937 Couldn't get mapping parameter : : = %d.\r\n", numMap + 1);
                        return 0xFF;
                      }
                        ...
                        if (objDict != OD_SUCCESSFUL)
                          {
                            MSG_ERR (0x1938,
                                     ...
                                     numMap + 1);
  						    //printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1938 error accessing to the mapped var  : = %d.\r\n", numMap + 1);
                    ...
                    if (d->PDO_status[numPdo].
                    ...
                    else
                      {
                        ...
                        MSG_ERR (0x1947,
                                 ...
                                 UNS16_LE(m->cob_id));
  						    //printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1947 Not ready RTR_SYNC TPDO send current data  : = %d.\r\n", UNS16_LE(m->cob_id));
                        status = state5;
                      }
                    break;
                  }
                  ...
                  if (buildPDO (d, numPdo, &pdo))
                    {
                      MSG_ERR (0x1948, " Couldn't build TPDO number : ", numPdo);
  						    //printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1948 Couldn't build TPDO number   : = %d.\r\n", numPdo);
                      return 0xFF;
                    }
  ...
  if (buildPDO (d, pdoNum, &pdo))
    {
      MSG_ERR (0x3907, " Couldn't build TPDO number : ",
               pdoNum);
  	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x3907 Couldn't build TPDO number   : = %d.\r\n", pdoNum);
      return 0;
    }
      ...
      while (offsetObjdict <= lastIndex)
        {
          switch (status)
                  ...
                  if (buildPDO (d, pdoNum, &pdo))
                    {
                      MSG_ERR (0x1906, " Couldn't build TPDO number : ",
                               pdoNum);
  						//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1906 Couldn't build TPDO number   : = %d.\r\n", pdoNum);
                      status = state11;
                      break;
                    }
                  status = state5;
                  ...
                  if (buildPDO
                      (d, pdoNum, &d->PDO_status[pdoNum].last_message))
                    {
                      MSG_ERR (0x1966, " Couldn't build TPDO number : ",
                               pdoNum);
  						//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1966 Couldn't build TPDO number   : = %d.\r\n", pdoNum);
                      d->PDO_status[pdoNum].transmit_type_parameter &=
                        ~PDO_RTR_SYNC_READY;
                    }
                  else
            ...
            default:
  			...
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1972 Unknown state has been reached    : = %d.\r\n", status);
              return 0xFF;
            }                   /* end switch case */
