...
void SDOTimeoutAlarm(CO_Data* d, UNS32 id)
{
	...
	MSG_ERR(0x1A01, "SDO timeout. SDO response not received.", 0);
  			//printf("error 0x1A01 SDO timeout. SDO response not received.  : = %d.\r\n", 0);
...
UNS8 lineToSDO (CO_Data* d, UNS8 line, UNS32 nbBytes, UNS8* data) {
	...
	if ((d->transfers[line].offset + nbBytes) > SDO_MAX_LENGTH_TRANSFER) {
		MSG_ERR(0x1A10,"SDO Size of data too large. Exceed SDO_MAX_LENGTH_TRANSFER", nbBytes);
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__); //printf("error 0x1A10 SDO Size of data too large. Exceed SDO_MAX_LENGTH_TRANSFER : = %d.\r\n", nbBytes); 
		return 0xFF;
	}
#endif //SDO_DYNAMIC_BUFFER_ALLOCATION
	...
	if ((d->transfers[line].offset + nbBytes) > d->transfers[line].count) {
		MSG_ERR(0x1A11,"SDO Size of data too large. Exceed count", nbBytes);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);	//printf("error 0x1A11 SDO Size of data too large. Exceed count : = %d.\r\n", nbBytes);
		return 0xFF;
	}
	...
	else
	{
		if (d->transfers[line].dynamicData == NULL)
		{
			MSG_ERR(0x1A11,"SDO's dynamic buffer not allocated. Line", line);
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A11 SDO's dynamic buffer not allocated. Line: = %d.\r\n", line);
			return 0xFF;
		}
		for (i = 0 ; i < nbBytes ; i++)
...
UNS8 SDOtoLine (CO_Data* d, UNS8 line, UNS32 nbBytes, UNS8* data)
{
	...
	if ((d->transfers[line].offset + nbBytes) > SDO_MAX_LENGTH_TRANSFER) {
		MSG_ERR(0x1A15,"SDO Size of data too large. Exceed SDO_MAX_LENGTH_TRANSFER", nbBytes);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);	//printf("error 0x1A15 SDO Size of data too large. Exceed SDO_MAX_LENGTH_TRANSFER = %d.\r\n", nbBytes);
		return 0xFF;
	}
#endif //SDO_DYNAMIC_BUFFER_ALLOCATION
			...
			if (d->transfers[line].dynamicData == NULL) {
				...
				if (d->transfers[line].dynamicData == NULL) {
					MSG_ERR(0x1A15,"SDO allocating dynamic buffer failed, size", SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A15 SDO allocating dynamic buffer failed, size = %d.\r\n", SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE);
					return 0xFF;
				}
				//Copy present data
			...
			else if ((d->transfers[line].offset + nbBytes) > d->transfers[line].dynamicDataSize)
			{
				...
				if (newDynamicBuffer == NULL) {
					MSG_ERR(0x1A15,"SDO reallocating dynamic buffer failed, size", d->transfers[line].dynamicDataSize + SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A15 SDO reallocating dynamic buffer failed, size= %d.\r\n", SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE);
					return 0xFF;
				}
...
{
	UNS32 i;
	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("0x3A25 reset SDO line nb = %d.\r\n",line);
...
{
	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("0x3A25 init SDO line nb = %d.\r\n",line);
...
UNS8 getSDOfreeLine ( CO_Data* d, UNS8 whoami, UNS8 *line )
{
	...
	MSG_ERR(0x1A25, "Too many SDO in progress. Aborted.", i);
  	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A25 Too many SDO in progress. Aborted.= %d.\r\n", i);
	return 0xFF;
}
...
UNS8 getSDOlineOnUse (CO_Data* d, UNS8 CliServNbr, UNS8 whoami, UNS8 *line)
{
	...
	UNS8 i;
	//printf("getSDOlineOnUse: CliServNbr = %d, whoami = %d, *line = %d \r\n", \
	//		CliServNbr,  whoami, *line);
	//printf("getSDOlineOnUse: d->transfers[i].state = %d,d->transfers[i].CliServNbr = %c, d->transfers[i].whoami = %c  \r\n",\
	//		d->transfers[i].state,  d->transfers[i].CliServNbr, d->transfers[i].whoami);
	for (i = 0 ; i < SDO_MAX_SIMULTANEOUS_TRANSFERS ; i++){
		if ( (d->transfers[i].state != SDO_RESET) &&
...
UNS8 closeSDOtransfer (CO_Data* d, UNS8 nodeId, UNS8 whoami)
{
	...
	if (err) {
	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("0x2A30 No SDO communication to close = 0.\r\n");
...
UNS8 setSDOlineRestBytes (CO_Data* d, UNS8 line, UNS32 nbBytes)
{
	...
	if (nbBytes > SDO_MAX_LENGTH_TRANSFER) {
  	...
  	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A35 SDO Size of data too large. Exceed SDO_MAX_LENGTH_TRANSFER = %d.\r\n", nbBytes);
		return 0xFF;
	}
#endif //SDO_DYNAMIC_BUFFER_ALLOCATION
...
UNS8 sendSDO (CO_Data* d, UNS8 whoami, UNS8 CliServNbr, UNS8 *pData)
{
	...
	MSG_WAR(0x3A38, "sendSDO",0);
  	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("MSG_WAR 0x3A38 d->nodeState = %d.\r\n", d->nodeState);
	if( !((d->nodeState == Operational) ||  (d->nodeState == Pre_operational ))) {
		MSG_WAR(0x2A39, "unable to send the SDO (not in op or pre-op mode", d->nodeState);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("MSG_WAR 0x2A39 unable to send the SDO (not in op or pre-op mode = %d.\r\n", d->nodeState);
		return 0xFF;
	}
	...
	if ( whoami == SDO_SERVER )	{
		...
		if ((offset == 0) || ((offset+CliServNbr) > d->lastIndex->SDO_SVR)) {
			MSG_ERR(0x1A42, "SendSDO : SDO server not found", 0);
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A42 SendSDO : SDO server not found = %d.\r\n", 0);
			return 0xFF;
		}
		...
		if ((offset == 0) || ((offset+CliServNbr) > d->lastIndex->SDO_CLT)) {
			MSG_ERR(0x1A42, "SendSDO : SDO client not found", 0);
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A42 SendSDO : SDO client not found = %d.\r\n", 0);
			return 0xFF;
		}
		...
		MSG_WAR(0x3A41, "I am client Tx cobId : ", m.cob_id);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("MSG_WAR 0x3A41 I am client Tx cobId  = %d.\r\n", m.cob_id);
	}
...
UNS8 proceedSDO (CO_Data* d, Message *m)
{
	...
	if(offset) while (offset <= lastIndex) {
  	...
  	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A61 Subindex 1  not found at index = %d.\r\n", 0x1200 + j);
			return 0xFF;
		}
	...
	if (whoami == SDO_UNKNOWN) {
  	...
  	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A63 Subindex 3  not found at index= %d.\r\n", 0x1280 + j);
				return 0xFF;
			}
			/* Looking for the cobid received. */
	...
	if ( (*m).len != 8) {
  	...
  	//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A67 Error size SDO= %d.\r\n", 0);
	...
	cs = 0xFF; 
	/* Special cases for block transfer : in frames with segment data cs is not sp茅cified */
   	if (!err) {
	...
	switch (cs) {
			...
			if (whoami == SDO_SERVER) {
				...
				if (err) {
					MSG_ERR(0x1A70, "SDO error : Received download segment for unstarted trans. index 0x1200 + ",
							CliServNbr);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A70 SDO error : Received download segment for unstarted trans. index 0x1200 + = %d.\r\n", CliServNbr);
				...
				if (d->transfers[line].toggle != getSDOt(m->data[0])) {
					MSG_ERR(0x1A72, "SDO error : Toggle error : ", getSDOt(m->data[0]));
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A72 SDO error : Toggle error : = %d.\r\n", getSDOt(m->data[0]));
				...
				if (getSDOc(m->data[0])) {
					...
					if (errorCode) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A54 SDO error : Unable to copy the data in the object dictionary: = %d.\r\n", 0);
				...
				if (err) {
					MSG_ERR(0x1A75, "SDO error : Received segment response for unknown trans. from nodeId", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A75 SDO error : Received segment response for unknown trans. from nodeId = %d.\r\n", nodeId);
					failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
					return 0xFF;
				}
				/* Reset the wathdog */
				...
				if (d->transfers[line].toggle != getSDOt(m->data[0])) {
					MSG_ERR(0x1A76, "SDO error : Received segment response Toggle error. from nodeId", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A76 SDO error : Received segment response Toggle error. from nodeId = %d.\r\n", nodeId);
					failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_TOGGLE_NOT_ALTERNED);
					return 0xFF;
				}
			...
			if (whoami == SDO_SERVER) {
				...
				if (! err) {
					MSG_ERR(0x1A81, "SDO error : Transmission yet started.", 0);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A81 SDO error : Transmission yet started. = %d.\r\n", 0);
				...
				if (err) {
					MSG_ERR(0x1A82, "SDO error : No line free, too many SDO in progress. Aborted.", 0);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A82 SDO error : No line free, too many SDO in progress. Aborted = %d.\r\n", 0);
					failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
					return 0xFF;
				}
				...
				if (getSDOe(m->data[0])) { /* If SDO expedited */
					...
					if (errorCode) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A84 SDO error : Unable to copy the data in the object dictionary = %d.\r\n", 0);
						failedSDO(d, CliServNbr, whoami, index, subIndex, errorCode);
						return 0xFF;
					}
					/* Release of the line. */
				...
				if (err) {
					MSG_ERR(0x1A85, "SDO error : Received segment response for unknown trans. from nodeId", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A85 SDO error : Received segment response for unknown trans. from nodeId= %d.\r\n", nodeId);
					failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
					return 0xFF;
				}
				/* Reset the wathdog */
				...
				if (d->transfers[line].toggle != getSDOt(m->data[0])) {
					MSG_ERR(0x1A86, "SDO error : Received segment response Toggle error. from nodeId", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A86 SDO error : Received segment response Toggle error. from nodeId= %d.\r\n", nodeId);
					failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_TOGGLE_NOT_ALTERNED);
					return 0xFF;
				}
			...
			if (whoami == SDO_SERVER) {
				...
				if (! err) {
					MSG_ERR(0x1A92, "SDO error : Transmission yet started at line : ", line);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A92 SDO error : Transmission yet started at line :  = %d.\r\n", line);
				...
				if (err) {
					MSG_ERR(0x1A71, "SDO error : No line free, too many SDO in progress. Aborted.", 0);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A71 SDO error : No line free, too many SDO in progress. Aborted.:  = %d.\r\n", 0);
					failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
					return 0xFF;
				}
				...
				if (errorCode) {
					MSG_ERR(0x1A94, "SDO error : Unable to copy the data from object dictionary. Err code : ",
							errorCode);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A94 SDO error : Unable to copy the data from object dictionary. Err code :  = %d.\r\n", errorCode);
					failedSDO(d, CliServNbr, whoami, index, subIndex, errorCode);
					return 0xFF;
				}
				...
				if (err) {
					MSG_ERR(0x1A97, "SDO error : Received response for unknown upload request from nodeId", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A97 SDO error : Received response for unknown upload request from nodeId  = %d.\r\n", nodeId);
					failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
					return 0xFF;
				}
				/* Reset the wathdog */
			...
			if (whoami == SDO_SERVER) {
				...
				if (err) {
					MSG_ERR(0x1AA0, "SDO error : Received upload segment for unstarted trans. index 0x1200 + ",
							CliServNbr);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AA0 SDO error : Received upload segment for unstarted trans. index 0x1200 +  = %d.\r\n", CliServNbr);
					failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
					return 0xFF;
				}
				/* Reset the wathdog */
				...
				if (d->transfers[line].toggle != getSDOt(m->data[0])) {
					MSG_ERR(0x1AA2, "SDO error : Toggle error : ", getSDOt(m->data[0]));
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AA2 SDO error : Toggle error :  = %d.\r\n", getSDOt(m->data[0]));
					failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_TOGGLE_NOT_ALTERNED);
					return 0xFF;
				}
				...
				if (err) {
					MSG_ERR(0x1AA5, "SDO error : Received response for unknown download request from nodeId", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AA5 SDO error : Received response for unknown download request from nodeId = %d.\r\n", nodeId);
					failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
					return 0xFF;
				}
				/* Reset the watchdog */
			...
			if (whoami == SDO_SERVER) { /* Server block upload */
				if (SubCommand == SDO_BCS_INITIATE_UPLOAD_REQUEST) {
				    ...
				    if (! err) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A93 SDO error : Transmission yet started at line  = %d.\r\n", line);
					...
					if(m->data[4] > 127){
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A96 SDO error : invalid block size  = %d.\r\n", 0);
				    ...
				    if (err) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A73 SDO error : No line free, too many SDO in progress. Aborted = %d.\r\n", 0);
					    failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
					    return 0xFF;
				    }
				    ...
				    if (errorCode) {
					    MSG_ERR(0x1A95, "SDO error : Unable to copy the data from object dictionary. Err code : ",
							errorCode);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A95 SDO error : Unable to copy the data from object dictionary. Err code  = %d.\r\n", errorCode);
					    failedSDO(d, CliServNbr, whoami, index, subIndex, errorCode);
					    return 0xFF;
				    }
 				    /* Preparing the response.*/
				    ...
				    if (err) {
					    MSG_ERR(0x1AA1, "SDO error : Received block upload request for unstarted trans. index 0x1200 + ",
							    CliServNbr);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AA1 SDO error : Received block upload request for unstarted trans. index 0x1200 +   = %d.\r\n", CliServNbr);
					    failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
					    return 0xFF;
				    }
                    /* Release the line */
				    ...
				    if (err) {
					    MSG_ERR(0x1AA1, "SDO error : Received block upload response for unstarted trans. index 0x1200 + ",
							    CliServNbr);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AA1 SDO error : Received block upload response for unstarted trans. index 0x1200 +   = %d.\r\n", CliServNbr);
					    failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
					    return 0xFF;
				    }
				    /* Reset the wathdog */
                    ...
                    if (SubCommand == SDO_BCS_UPLOAD_RESPONSE) {
                        ...
                        if(d->transfers[line].offset > d->transfers[line].count) { /* Bad AckSeq reveived (too high) */
					        MSG_ERR(0x1AA1, "SDO error : Received upload response with bad ackseq index 0x1200 + ",
							    CliServNbr);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AA1 SDO error : Received upload response with bad ackseq index 0x1200 +    = %d.\r\n", CliServNbr);
					        failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
					        return 0xFF;
                        }
           			}
                ...
                if ((SubCommand == SDO_BSS_INITIATE_DOWNLOAD_RESPONSE) || (SubCommand == SDO_BSS_DOWNLOAD_RESPONSE)) {
                    ...
                    if (err) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAA SDO error : Received response for unknown block download request from node id = %d.\r\n", nodeId);
                        failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
                        return 0xFF;
                    }
                    index = d->transfers[line].index;
                    ...
                    if (SubCommand == SDO_BSS_INITIATE_DOWNLOAD_RESPONSE) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A98 SDO error : invalid block size = %d.\r\n", 0);
					    	failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_INVALID_BLOCK_SIZE);
					    	return 0xFF;
						}
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A99 SDO error : invalid block size = %d.\r\n", 0);
					    	failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_INVALID_BLOCK_SIZE);
					    	return 0xFF;
						}
                   		d->transfers[line].blksize = m->data[2];
                        ...
                        if(d->transfers[line].offset > d->transfers[line].count) { /* Bad AckSeq reveived (too high) */
					        MSG_ERR(0x1AA1, "SDO error : Received upload segment with bad ackseq index 0x1200 + ",
							    CliServNbr);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AA1 SDO error : Received upload segment with bad ackseq index 0x1200 +  = %d.\r\n", 0);
					        failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
					        return 0xFF;
                        }
					}
			    	...
			    	MSG_ERR(0x1AAB, "SDO error block download : Received wrong subcommand from nodeId", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAB SDO error block download : Received wrong subcommand from nodeId = %d.\r\n", nodeId);
    				failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
	    			return 0xFF;
				}
			}      /* end if CLIENT */
			...
			if (whoami == SDO_SERVER) { /* Server block download */
				if (err) {
					...
					if (SubCommand != SDO_BCS_INITIATE_DOWNLOAD_REQUEST) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAC SDO error block download : Received wrong subcommand from node = %d.\r\n", nodeId);
    				    failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
	    			    return 0xFF;
				    }
					...
					if (err) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1A89 SDO error No line free, too many SDO in progress. Aborted = %d.\r\n", 0);
						failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
						return 0xFF;
					}
					...
					if ((m->data[0] & 1) != SDO_BCS_END_DOWNLOAD_REQUEST) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAD SDO error block download : Received wrong subcommand - index 0x1200 + = %d.\r\n", CliServNbr);
    					failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
	    				return 0xFF;
					}
    		    	RestartSDO_TIMER(line)
					...
					if(d->transfers[line].objsize){ /* If size was indicated in the initiate request */
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAE SDO error block download : sizes do not match - index 0x1200 + = %d.\r\n", CliServNbr);
					...
					if (errorCode) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAF SDO error Unable to copy the data in the object dictionary = %d.\r\n", 0);
				...
				if (err) {
			    	...
			    	MSG_ERR(0x1AAD, "SDO error block upload : no transmission started", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAD SDO error block upload : no transmission started = %d.\r\n", nodeId);
    				failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
	    			return 0xFF;
				}
    			RestartSDO_TIMER(line)
				if (d->transfers[line].rxstep == RXSTEP_INIT) {
					...
					if ((m->data[0] & 1) != SDO_BSS_END_UPLOAD_RESPONSE) {
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAD SDO error block upload : Received wrong subcommand from node id = %d.\r\n", nodeId);
    					failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
	    				return 0xFF;
					}
					NbBytesNoData = (UNS8)((m->data[0]>>2) & 0x07);
					...
					if(d->transfers[line].objsize){ /* If size was indicated in the initiate request */
  					...
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("error 0x1AAE SDO error block download : sizes do not match - from node id = %d.\r\n", nodeId);
    							failedSDO(d, CliServNbr, whoami, d->transfers[line].index, d->transfers[line].subIndex, SDOABT_LOCAL_CTRL_ERROR);
	    						return 0xFF;
						}
					}
		...
		default:
			...
			MSG_ERR(0x1AB2, "SDO. Received unknown command specifier : ", cs);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf("0x1AB2 error 0x1AB2 SDO. Received unknown command specifier :  = %d.\r\n", cs);
			return 0xFF;
...
UNS8 GetSDOClientFromNodeId( CO_Data* d, UNS8 nodeId )
{
	...
	if (offset == 0) {
		MSG_ERR(0x1AC6, "No SDO client index found for nodeId ", nodeId);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf(" error 0x1AC6 No SDO client index found for nodeId = %d.\r\n", nodeId);
		return 0xFF;
	}
	CliNbr = 0;
	while (offset <= lastIndex) {
		if (d->objdict[offset].bSubCount <= 3) {
			MSG_ERR(0x1AC8, "Subindex 3  not found at index ", 0x1280 + CliNbr);
  					//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);//printf(" error 0x1AC8 Subindex 3  not found at index  = %d.\r\n", 0x1280 + CliNbr);
			return 0xFF;
		}
	...
	if (!SDOfound) {
		MSG_WAR(0x1AC9, "SDO No preset client found to communicate with node : ", nodeId);
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);		//printf("error 0x1AC9 SDO No preset client found to communicate with node = %d.\r\n", nodeId);
		return 0xFE;
	}
		...
		UNS8 subIndex, UNS32 count, UNS8 dataType, void *data, SDOCallback_t Callback, UNS8 endianize, UNS8 useBlockMode)
{
	...
	if(count > SDO_MAX_LENGTH_TRANSFER){
		MSG_ERR(0x1AC3, "SDO error : request for more than SDO_MAX_LENGTH_TRANSFER bytes to transfer to node : ", nodeId);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);			//printf("error 0x1AC3 SDO error : request for more than SDO_MAX_LENGTH_TRANSFER bytes to transfer to node = %d.\r\n", nodeId);
		return 0xFF;
	}
#endif
	...
	if (!err) {
		MSG_ERR(0x1AC4, "SDO error : Communication yet established. with node : ", nodeId);
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);		//printf("error 0x1AC4 SDO error : Communication yet established. with node := %d.\r\n", nodeId);
		return 0xFF;
	}
	/* Taking the line ... */
	...
	if (err) {
		MSG_ERR(0x1AC5, "SDO error : No line free, too many SDO in progress. Aborted for node : ", nodeId);
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);		//printf("error 0x1AC5 SDO error : No line free, too many SDO in progress. Aborted for node : = %d.\r\n", nodeId);
		return (0xFF);
	}
	else {
		...
		if (count > SDO_MAX_LENGTH_TRANSFER)
		{
			...
			if (d->transfers[line].dynamicData == NULL)
			{
				MSG_ERR(0x1AC9, "SDO. Error. Could not allocate enough bytes : ", count);
  				//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);	//printf("error 0x1AC9 SDO. Error. Could not allocate enough bytes : = %d.\r\n", count);
				return 0xFE;
			}
			lineData = d->transfers[line].dynamicData;
		}
...
#  endif
	...
	if (err) {
		...
		for(int i = 0; i < 8 ;++i)
		{
			MSG_WAR(0x1AD1, "SDO. Error while sending SDO to node : ", buf[i]);
		}
		/* release the line */
		resetSDOline(d, line);
		return 0xFF;
	...
	if(ret == 0xFE)
	{
		...
		if (offset == 0)
		{
			MSG_ERR(0x1AC6, "writeNetworkDict : No SDO client index found", 0);
  			//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);		//printf("error 0x1AC6 writeNetworkDict : No SDO client index found= %d.\r\n", 0);
			return 0xFF;
		}
		i = 0;
		while (offset <= lastIndex)
...
INLINE UNS8 _readNetworkDict (CO_Data* d, UNS8 nodeId, UNS16 index, UNS8 subIndex, UNS8 dataType, SDOCallback_t Callback, UNS8 useBlockMode)
{
	...
	if (!err) {
		MSG_ERR(0x1AD8, "SDO error : Communication yet established. with node : ", nodeId);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);			//printf("error 0x1AD8 SDO error : Communication yet established. with node = %d.\r\n", nodeId);
		return 0xFF;
	}
	/* Taking the line ... */
	err = getSDOfreeLine( d, SDO_CLIENT, &line );
	if (err) {
		MSG_ERR(0x1AD9, "SDO error : No line free, too many SDO in progress. Aborted for node : ", nodeId);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);			//printf("error 0x1AD9 SDO error : No line free, too many SDO in progress. Aborted for node = %d.\r\n", nodeId);
		return (0xFF);
	}
	else {
	...
	if (err) {
		MSG_ERR(0x1AE5, "SDO. Error while sending SDO to node : ", nodeId);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);			//printf("error 0x1AE5 SDO Error while sending SDO to node = %d.\r\n", nodeId);
		/* release the line */
		resetSDOline(d, line);
		return 0xFF;
	}
	...
	* abortCode = 0;
	...
	if (err) {
		MSG_ERR(0x1AF0, "SDO error : No line found for communication with node : ", nodeId);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);			//printf("error 0x1AF0 SDO error : No line found for communication with node : = %d.\r\n", nodeId);
        *size = 0;
        return SDO_ABORTED_INTERNAL;
	}
	...
	* abortCode = 0;
	...
	if (err) {
		MSG_ERR(0x1AF1, "SDO error : No line found for communication with node : ", nodeId);
  		//printf("文件名:%s 行数：%d\n", __FILE__ , __LINE__);			//printf("error 0x1AF1 SDO error : No line found for communication with node = %d.\r\n", nodeId);
		return SDO_ABORTED_INTERNAL;
	}
	* abortCode = d->transfers[line].abortCode;
    if (d->transfers[line].state != SDO_FINISHED)
